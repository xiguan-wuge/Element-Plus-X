...
| `slotConfig`              | SlotConfigType[]     | 否       | undefined                  | **词槽模式**：传入后自动切换为 `contenteditable` 词槽编辑器。分段类型：`text`（纯文本）/ `content`（可编辑内容段）/ `input`（内嵌输入框）/ `select`（内嵌下拉）/ `tag`（固定标签）/ `custom`（自定义渲染 VNode）。每段均可选 `formatResult` 格式化输出值。 |
| `skill`                   | SkillType            | 否       | undefined                  | **词槽模式**：`@技能` 芯片，渲染在输入区最前方。`{ value, title?, toolTip?, closable? }`，`closable` 支持 `boolean` 或 `{ closeIcon?, onClose?, disabled? }`。                                                                                             |

## 事件

| 事件名            | 说明                                  | 回调参数                                                                                                           |
| ----------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `submit`          | 内置 `提交按钮` 提交时触发的事件。    | 词槽模式下返回 `(value: string, slotConfig?: SlotConfigType[], skill?: SkillType)`，文本模式返回 `(value: string)` |
...
| `focus`            | Function | 聚焦输入框。 默认 `focus('all')` 聚焦整个文本，`focus('start')` 聚焦文本最前方，`focus('end')` 聚焦文本最后方。词槽模式额外支持 `focus({ cursor: 'slot', key: 'xxx' })` 聚焦指定词槽。 |
| `insert`           | Function | **词槽模式**：插入内容。`insert(slots: SlotConfigType[], position: 'start' \| 'end' \| 'cursor', replaceCharacters?, preventScroll?)`，`slots` 为单段或多段配置。                      |
| `getValue`         | Function | **词槽模式**：获取结构化值，返回 `{ value: string, slotConfig: (SlotConfigType & { value?: string })[], skill?: SkillType }`。                                                         |
